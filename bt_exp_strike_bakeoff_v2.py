...
Policies compared (everything else held fixed â€” same entry timestamps, same
give-back exit engine with the live params +25 half / give-back 40 / -70 stop,
same approximate BS pricing, slippage, fees):

  first_otm   â€” current live rule: first strike at/above spot (calls),
                at/below spot (puts)
  second_otm  â€” one increment further OTM than first_otm
  delta40     â€” strike on the grid whose BS |delta| at entry is nearest 0.40
  delta25     â€” strike on the grid whose BS |delta| at entry is nearest 0.25
...
pricing â€” trust RELATIVE rankings only. Entries are the allow-list setups
...
Writes bt_exp_strike_bakeoff_v2.json (repo root â€” NOT reports/, which drives the
...
def collect_entries(cfg, intraday, daily):
    """Same entry logic as backtest_param_sweep.collect â€” strike NOT chosen yet."""
    rows = []
...
def main():
    ...
    for attempt in range(4):
        try:
            ...
            break
        except Exception as e:  # yfinance throttle â€” back off and retry
            if attempt == 3:
                raise
    ...
    print(f"\nSTRIKE BAKE-OFF â€” identical entries ({len(kept)}) and give-back "
    ...
    print("(approx BS pricing, optimistic for 0DTE â€” trust RELATIVE ranking; "
    ...
    out = {
        ...
        "pricing": "approximated (Black-Scholes, 20d realized vol) â€” optimistic for 0DTE",
    ...
    out_path = REPO_DIR / "bt_exp_strike_bakeoff_v2.json"
