"""bt_exp v2: PER-SETUP CONTRIBUTION over the freshest 60d of 5m data.
...
compares portfolio totals on IDENTICAL entries:

  A. full allow-list  {SPX:call, SPY:call, QCOM:call, TSLA:put}
  B. without TSLA:put
  C. TSLA:put at HALF size (0.5x dollar weight; win rate unchanged)

Drift slices for TSLA:put: first vs second half of the window, entries
after 2026-06-18 (bars the saved report never saw), and the last 10
sessions.

Experiment file only. Writes bt_exp_per_setup_contrib_v2.json in the repo
root (NEVER reports/). Approximate Black-Scholes pricing — trust RELATIVE
ranks only.
"""
...
OUT = Path(__file__).parent / "bt_exp_per_setup_contrib_v2.json"
ALLOWED = ("SPX:call", "SPY:call", "QCOM:call", "TSLA:put")
STOP_ABS = abs(config.STOP_PCT)  # 70 — same stop everywhere, sizing-fair divisor

# current live-config baseline quoted by the orchestrator (~60d, give-back-40)
BASELINE = {"SPX:call": (78, 31), "SPY:call": (78, 26),
            "QCOM:call": (71, 17), "TSLA:put": (67, 6)}
...
def main():
    ...
    if tsla_puts:
        ...
        sessions = sorted({t.entry_time.date() for t in seq})
        last10 = set(sessions[-10:])
        drift["last_10_sessions"] = metrics(
            [t for t in seq if t.entry_time.date() in last10])
    ...
    out = {
        ...
        "window": {tk: [str(df.index.min()), str(df.index.max())]
                   for tk, df in intraday.items()},
        "baseline_quoted": {k: {"win_rate": v[0], "expectancy_pct": v[1]}
